var
  ...
  procedure PlayGame; //Seçim panelini açar
  begin
    PanelChoice.Visible := True;
    ButtonBackPanel.Visible := True;
  end;
  procedure OpenCreateRoomPanel; //Oda Kurma Panelini Açar.
  begin
  ...
  procedure OpenLoginRoomPanel; //Odaya Katılma Panelini Açar.
  begin
    PanelRoom.Visible := True;
  ...
  procedure CreateOrJoinPassword; //Oda oluşturma veya Odaya katılma prosedürü.
  begin
    case EditPasword.cltagInt of
      0://Kol
      begin
       if(EditPasword.Text <> '')then
       begin
  ...
  procedure ResolutionControl; //Otomatik Ekran Çözünürlük Kontrolü.
  begin
    if(TForm(MyForm).ClientWidth < TForm(MyForm).ClientHeight)then //Ekran Dik çevrildiğinde.
    begin
    ...
    else if(TForm(MyForm).ClientWidth > TForm(MyForm).ClientHeight)then //Ekran Dik çevrildiğinde.
    begin
  ...
  procedure ClosePanel; //Panelleri kapatan prosedür.
  begin
    PanelChoice.Visible := False;
    PanelRoom.Visible := False;
    ButtonBackPanel.Visible := False;
  end;
  procedure CloseApplication; //Uygulamadan Çıkan prosedür.
  begin
